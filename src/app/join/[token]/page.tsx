...
// Maps the API role/reason enums to i18n keys. The labels themselves
// live in the dictionaries and are resolved via t() inside the
// component (where the translator is available).
const ROLE_KEY: Record<PeekOk['role'], string> = {
  admin: 'join.roles.admin',
  agent: 'join.roles.agent',
  viewer: 'join.roles.viewer',
};

const FAIL_KEY: Record<PeekFail['reason'], { title: string; body: string }> = {
  not_found: { title: 'join.fail.notFound.title', body: 'join.fail.notFound.body' },
  used: { title: 'join.fail.used.title', body: 'join.fail.used.body' },
  expired: { title: 'join.fail.expired.title', body: 'join.fail.expired.body' },
  server_error: {
    title: 'join.fail.serverError.title',
    body: 'join.fail.serverError.body',
  },
};
...
export default function JoinPage() {
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(payload.error || t('join.conflict.defaultMessage'));
        } else {
          toast.error(payload.error || t('join.accept.failed'));
        }
        setAccepting(false);
        return;
      }
      toast.success(t('join.accept.success'));
      // Full reload (not router.push) so AuthProvider re-fetches
      // the profile with the new account_id and account_role.
      window.location.href = '/dashboard';
    } catch (err) {
      console.error('[join] redeem error:', err);
      toast.error(t('join.accept.unreachable'));
      setAccepting(false);
    }
  }, [token, t]);
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('join.conflict.signOutError'));
      setSigningOut(false);
    }
  }, [t]);
          ...
          {peek.reason === 'server_error' ? (
            <>
              ...
              <Link href="/signup">
                ...
                >
                  {t('join.failActions.createAccountInstead')}
                </Button>
              </Link>
            </>
          ) : (
            ...
            >
              {accepting ? (
                <>
                  ...
                  {t('join.accept.accepting')}
                </>
              ) : (
                <>
                  <CheckCircle className="size-4" />
                  {t('join.accept.button')}
                </>
              )}
            </Button>
            <p className="text-center text-xs text-muted-foreground">
              {t('join.accept.note', { accountName: peek.account_name })}
            </p>
          </CardContent>
        </Card>
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {t('join.conflict.title', { accountName: peek.account_name })}
              </DialogTitle>
            ...
            <div className="space-y-2 py-2 text-xs text-muted-foreground">
              <p>{t('join.conflict.body', { accountName: peek.account_name })}</p>
            </div>
            <DialogFooter className="bg-popover border-border">
              ...
              >
                {signingOut ? (
                  ...
                  t('join.conflict.signOutAndUseDifferentEmail')
                )}
              </Button>
            </DialogFooter>
          </DialogContent>
    ...
    <Card className="w-full max-w-md border-border bg-card">
      ...
      <CardContent className="flex flex-col gap-2">
        <Link href={`/signup?invite=${encodeURIComponent(token!)}`}>
          <Button className="w-full bg-primary text-primary-foreground hover:bg-primary/90">
            {t('join.cta.createAccountAndJoin')}
          </Button>
        </Link>
