...
export function PipelineSettings({
  ...
  async function handleRemoveStage(stageId: string) {
    ...
    const { count } = await supabase
      ...
      .eq("stage_id", stageId);
    if (count && count > 0) {
      toast.error(t("pipelines.settings.toast.moveDealsFirst"));
      return;
    }
  ...
  async function handleDeletePipeline() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t("pipelines.settings.toast.deletePipelineFailed"));
      return;
    }
    onOpenChange(false);
    onPipelinesChanged();
        ...
        {showDeleteConfirm ? (
          <div className="py-4">
            ...
            <div className="mt-4 flex justify-end gap-2">
              ...
              >
                {deleting ? t("common.deleting") : t("pipelines.settings.deletePipeline")}
              </Button>
            </div>
          </div>
        ) : (
          <>
            <div className="grid gap-4 py-2">
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("pipelines.settings.pipelineName")}</Label>
                <Input
                  value={name}
            ...
            <DialogFooter className="border-border bg-popover/50">
              ...
              >
                {saving ? t("common.saving") : t("pipelines.settings.saveChanges")}
              </Button>
            </DialogFooter>
          </>
        )}
