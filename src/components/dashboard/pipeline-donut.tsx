              ...
              {data.stages.map((s) => (
                <li key={s.id} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="text-muted-foreground tabular-nums">
                    {t(
                      s.dealCount === 1
                        ? 'dashboard.pipeline.deal'
                        : 'dashboard.pipeline.deals',
                      { count: s.dealCount },
                    )}
                  </span>
...
// ------------------------------------------------------------
function Donut({ data, currency, t }: { data: PipelineDonutData; currency: string; t: Translator }) {
  const size = 200
  const r = 80
  const ringWidth = 18
  const cx = size / 2
  ...
  return (
    <div className="flex items-center justify-center">
      <svg viewBox={`0 0 ${size} ${size}`} className="h-48 w-48" role="img" aria-label={t('dashboard.pipeline.ariaLabel')}>
        {/* background ring */}
