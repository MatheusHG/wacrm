...
export function PasswordForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!profile?.email) {
      toast.error(t('settings.password.noEmailError'));
      return;
    }
    ...
    try {
      ...
      if (signInError) {
        toast.error(t('settings.password.currentIncorrect'));
        return;
      }
      ...
      if (updateError) {
        toast.error(t('settings.password.updateFailed', { message: updateError.message }));
        return;
      }
