...
// Maps a pathname to a `nav.items.*` translation key; the title itself
// is resolved through `t()` so it follows the active locale.
...
export function Header({ onOpenSidebar }: HeaderProps) {
  ...
  const { t } = useI18n();
  const title = t(`nav.items.${getPageTitleKey(pathname)}`);
