...
const contactFields = [
  { value: 'name', labelKey: 'broadcasts.step3.contactFields.name' },
  { value: 'phone', labelKey: 'broadcasts.step3.contactFields.phone' },
  { value: 'email', labelKey: 'broadcasts.step3.contactFields.email' },
  { value: 'company', labelKey: 'broadcasts.step3.contactFields.company' },
];
    ...
    <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div>
                    <label className="mb-1.5 block text-xs font-medium text-muted-foreground">
                      {t('broadcasts.step3.mappingType')}
                    </label>
                    <Select
                      value={mapping.type}
                      onValueChange={(val) =>
                    ...
                    >
                      ...
                      <SelectContent className="border-border bg-popover">
                        <SelectItem value="static">{t('broadcasts.step3.mappingTypes.static')}</SelectItem>
                        <SelectItem value="field">{t('broadcasts.step3.mappingTypes.field')}</SelectItem>
                        <SelectItem value="custom_field">
                          {t('broadcasts.step3.mappingTypes.customField')}
                        </SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-medium text-muted-foreground">
                      {mapping.type === 'static' ? t('broadcasts.step3.valueLabel') : t('broadcasts.step3.fieldLabel')}
                    </label>
                    {mapping.type === 'static' ? (
                      <Input
                        value={mapping.value}
                        ...
                        placeholder={t('broadcasts.step3.valuePlaceholder')}
                      ...
                      >
                        <SelectTrigger className="w-full border-border bg-muted text-foreground">
                          <SelectValue
                            placeholder={
                              loadingFields
                                ? t('broadcasts.step3.customFieldLoading')
                                : customFields.length === 0
                                  ? t('broadcasts.step3.customFieldNone')
                                  : t('broadcasts.step3.customFieldSelect')
                            }
                          />
                        </SelectTrigger>
                        <SelectContent className="border-border bg-popover">
      ...
      <div className="rounded-xl border border-border bg-card/50 p-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <p className="text-sm font-medium text-foreground">{t('broadcasts.step3.livePreview')}</p>
