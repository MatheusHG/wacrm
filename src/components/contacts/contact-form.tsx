...
export function ContactForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!phone.trim()) {
      toast.error(t('contacts.form.toast.phoneRequired'));
      return;
    }
    ...
    if (!isEdit && dupMatch?.exact) {
      toast.error(t('contacts.form.toast.duplicateExists'));
      return;
    }
    ...
    try {
      ...
      if (!user) throw new Error(t('contacts.form.toast.notAuthenticated'));
      if (!accountId) throw new Error(t('contacts.form.toast.noAccount'));
      ...
      toast.success(isEdit ? t('contacts.form.toast.updated') : t('contacts.form.toast.created'));
      onOpenChange(false);
      onSaved();
    } catch (err: unknown) {
      ...
      const message = err instanceof Error ? err.message : t('contacts.form.toast.saveFailed');
      toast.error(message);
    } finally {
      setSaving(false);
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="cf-phone" className="text-muted-foreground">
              {t('contacts.form.fields.phone')} <span className="text-red-400">*</span>
            </Label>
            <Input
              id="cf-phone"
              value={phone}
            ...
            {dupMatch ? (
              ...
              >
                ...
                <div className="space-y-1">
                  <p>
                    {dupMatch.exact
                      ? t('contacts.form.dupExact')
                      : t('contacts.form.dupSimilar')}
                  </p>
                  {onViewExisting && (
                    <button
                      type="button"
          ...
          <div className="space-y-2">
            <Label htmlFor="cf-company" className="text-muted-foreground">
              {t('contacts.form.fields.company')}
            </Label>
            <Input
              id="cf-company"
              value={company}
          ...
          <div className="space-y-2">
            <Label className="text-muted-foreground">{t('contacts.form.fields.tags')}</Label>
            {loadingTags ? (
