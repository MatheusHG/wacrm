...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (!name) return;
    if (!accountId || !user) {
      toast.error(t('contacts.customFields.toast.noAccount'));
      return;
    }
    if (isDuplicate(name)) {
      toast.error(t('contacts.customFields.toast.duplicate', { name }));
      return;
    }
    ...
    if (error) {
      toast.error(t('contacts.customFields.toast.createFailed'));
      return;
    }
    toast.success(t('contacts.customFields.toast.created', { name }));
    setNewName('');
    await fetchFields();
  }
  ...
  async function handleRename(
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.customFields.toast.renameFailed'));
      return false;
    }
    await fetchFields();
    return true;
  ...
  async function handleDelete(field: CustomField) {
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('contacts.customFields.toast.deleteFailed'));
      return;
    }
    toast.success(t('contacts.customFields.toast.deleted', { name: field.field_name }));
    await fetchFields();
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          placeholder={t('contacts.customFields.newPlaceholder')}
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      <Input
        ...
        aria-label={t('contacts.customFields.renameLabel', { name: field.field_name })}
