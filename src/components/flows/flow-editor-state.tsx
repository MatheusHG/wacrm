...
export function FlowEditorProvider({
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      setDirty(false);
      toast.success(t("flows.state.saved"));
    } catch (err) {
      const msg = err instanceof Error ? err.message : t("flows.state.saveFailed");
      toast.error(msg);
    } finally {
      setSaving(false);
    }
  ...
  const setStatus = useCallback(
    async (next: BuilderState["status"]) => {
      if (next === "active" && !canActivate) {
        toast.error(t("flows.state.fixIssuesBeforeActivating"));
        return;
      }
      setActivating(true);
      try {
        ...
        toast.success(
          next === "active"
            ? t("flows.state.flowActivated")
            : next === "archived"
              ? t("flows.state.archived")
              : t("flows.state.savedAsDraft"),
        );
      } catch (err) {
        const msg =
          err instanceof Error
            ? err.message
            : t("flows.state.statusUpdateFailed");
        toast.error(msg);
      } finally {
        setActivating(false);
      }
  ...
  const deleteFlow = useCallback(async () => {
    const yes = window.confirm(
      t("flows.state.deleteConfirm", { name: state.name }),
    );
    if (!yes) return;
    try {
      const res = await fetch(`/api/flows/${initialFlow.id}`, {
      ...
      const msg = err instanceof Error ? err.message : t("flows.state.deleteFailed");
      toast.error(msg);
    }
  ...
  const addNode = useCallback(
    (type: NodeType): string => {
      // node_key is a stable, locale-independent identifier — derive it
      // from the node type, not the (now-translated) display label.
